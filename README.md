...
#1.アプリ概要
登録したアイテムを*1綺麗め度・*2暑さ度に基づいてコーデを自動生成するwebアプリケーションです。

*1綺麗め度:アイテムのフォーマル度合いを指します。1から9の値で評価し、最大の9にスーツが該当すると僕は考えています。  
*2暑さ度:アイテムがどれくらい暖かいのかを指します。1から9の値で評価し、最大の9に厚手のダウンジャケットが該当すると僕は考えています。  

私は洋服が大好きで、いろんなジャンルの洋服を着ます。  
よく服を着ていく中で、以下のような困りごとがありました。  
...
itemデーブルのclor_differenceカラムは、画像の占める割合が一番大きい色のrgb(red,green,blue)の最大と最小値の差を保持します。アイテム登録時にCloud Vision APIにて画像の占める割合が一番大きい色をjson形式でrgbを取得します。rgbの最大値と最小値をccolor_differenceメソッドで計算しccolor_differenceカラムに格納しています。
